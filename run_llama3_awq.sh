#!/bin/bash
#SBATCH -J job_id
#SBATCH -o ./log/test_llama31_instruct_awq_lora.out
...
SIF_PATH="./img/sdft_awq.img"  # or .img if that's your extension
...
# test compressed seed lm
#singularity exec --nv "$SIF_PATH" bash scripts_llama3/test_seed_compressed_LM.sh

# vanilla ft compresssed
#singularity exec --nv "$SIF_PATH" bash scripts_llama3/openfunction/sft_compressed.sh

# sdft 
singularity exec --nv "$SIF_PATH" bash scripts_llama3/openfunction/sdft_compressed.sh

# test pr seed lm
#singularity exec --nv "$SIF_PATH" bash scripts_llama3/test_seed_pr_LM.sh